...
test('1. Headings add sentence pause if missing punctuation', async () => {
...
test('2. Headings preserve existing punctuation', async () => {
...
test('3. Markdown links keep display text and drop URLs', async () => {
...
test('6. Markdown images and Obsidian embeds are completely removed', async () => {
...
test('7. Inline formatting markers are stripped without affecting words', async () => {
...
test('8. Cloze questions - active test mode (revealAnswers = false)', async () => {
...
test('9. Cloze questions - review mode (revealAnswers = true)', async () => {
...
test('10. Cloze questions - Anki format c1::answer::hint', async () => {
...
test('11. HTML tags are stripped and block tags create sentence pauses', async () => {
...
test('12. HTML entities are decoded naturally by language', async () => {
...
test('16. Code blocks and inline code', async () => {
    const raw = 'Kodu çalıştırınız: `const x = 10;` ve devam ediniz.';
    const cleaned = cleanTextForSpeech(raw);
    assert.equal(cleaned, 'Kodu çalıştırınız: const x = 10; ve devam ediniz.');
});

test('17. Max length limit guards against URL overflow at sentence boundary', async () => {
...
test('18. Null, empty, and non-string inputs return empty string safely', async () => {
