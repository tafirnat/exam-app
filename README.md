...
- **Automatic folder on import**: Ask the AI to put a set into its own folder and it writes `"folder": "ITIL 4 Foundation"` into `exam_metadata`. On import the set goes into the folder with that name — reused if it exists (case and accents don't matter), created if it doesn't. Matching is by **name**, so the file needs no knowledge of your folder ids, and every part of a series lands in the same folder. Opt-in: switch on **Apply folder hints** below *Paste from Clipboard* (off by default; with it off the field is ignored). Applied on first import only — move the set later and it stays moved. When you share a set you can choose to include its folder name.
- **Folder by id**: Alternatively, `"folderId": "folder_..."` targets an existing folder directly (copy the id via the `id: ...` button in the folder management modal); it takes precedence over `folder`.
