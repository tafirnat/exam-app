    ...
    for (const block of blocks) {
        switch (block.type) {
            case 'code_block': {
                ...
                const codeTrimmed = (block.code || '').trim();
                const isLong = codeTrimmed.length >= 40 || (block.code && block.code.includes('\n') && codeTrimmed.length >= 25);
                const copyBtn = isLong
                    ? `<button type="button" class="md-code-copy-btn" title="Kopieren" aria-label="Kopieren"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg><span class="copy-text">Kopieren</span></button>`
                    : '';
                htmlParts.push(`<pre><code${classAttr}>${escapedCode}</code>${copyBtn}</pre>`);
                break;
            }
