...
// document wins for every later token.
...
function createTokenSvg(tokenIndex, active) {
    ...
    // 1st snowflake (tokenIndex 0): Luminous ice blue (Mavi Kar Tanesi)
    ...
    if (tokenIndex === 1) {
        ...
        tokenLabel = `2. Kızıl Kar Tanesi — ${active ? 'Aktif' : 'Pasif'}`;
    } else {
        iceColor = active ? '#7dd3fc' : 'var(--text-secondary)';
        tipColor = iceColor;
        tokenLabel = `1. Mavi Kar Tanesi — ${active ? 'Aktif' : 'Pasif'}`;
    }
    ...
    // Both tokens share one snowflake outline, so colour alone has to carry the
    // difference. The joker gets a radial gradient — crimson core bleeding out
    // to flame orange at the arm tips — while the gift token stays flat ice blue.
    let fillRef = iceColor;
    let defsHtml = '';
    if (tokenIndex === 1 && active) {
        const gradientId = `focusTokenFlame${++tokenGradientSeq}`;
        defsHtml =
            `<defs><radialGradient id="${gradientId}" cx="50%" cy="50%" r="55%">` +
            `<stop offset="30%" stop-color="${iceColor}"/>` +
            `<stop offset="100%" stop-color="${tipColor}"/>` +
            `</radialGradient></defs>`;
...
export function showSingleTokenModal(type, tokenIndex) {
    ...
    if (tokenIndex === 0) {
        ...
        const title = `<span style="color: #7dd3fc;">❄️</span> 1. Mavi Kar Tanesi`;
        const html = `
            <div style="font-size: 0.88rem; color: var(--text-primary); text-align: left; display: flex; flex-direction: column; gap: 0.85rem;">
                <div style="display: flex; align-items: center; justify-content: space-between; background: rgba(125, 211, 252, 0.1); padding: 0.75rem 0.9rem; border-radius: 8px; border-left: 3px solid #7dd3fc;">
                    <div>
                        <strong style="color: #7dd3fc; font-size: 0.92rem; display: block;">Mavi Kar Tanesi</strong>
        ...
        const active = isGlobal ? false : true;
        const title = `<span style="color: #ef4444;">❄️</span> 2. Kızıl Kar Tanesi`;
        const html = `
            <div style="font-size: 0.88rem; color: var(--text-primary); text-align: left; display: flex; flex-direction: column; gap: 0.85rem;">
                <div style="display: flex; align-items: center; justify-content: space-between; background: rgba(239, 68, 68, 0.08); padding: 0.75rem 0.9rem; border-radius: 8px; border-left: 3px solid #ef4444;">
                    <div>
                        <strong style="color: #ef4444; font-size: 0.92rem; display: block;">Kızıl Kar Tanesi</strong>
                        <span style="font-size: 0.78rem; color: var(--text-secondary);">Ait Olduğu Seri: ${seriesTitle}</span>
                    </div>
            ...
            <div style="font-size: 0.88rem; color: var(--text-primary); text-align: left; display: flex; flex-direction: column; gap: 0.85rem;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.65rem;">
                    <div style="display: flex; gap: 0.6rem; align-items: flex-start;">
                        ...
                        <div>
                            <strong style="font-size: 0.88rem; color: var(--text-primary);">Seri Nasıl Oluşturulur?</strong>
                            <p style="margin: 0.15rem 0 0 0; font-size: 0.83rem; color: var(--text-secondary); line-height: 1.45;">
                                Her gün FSRS tarafından hesaplanan günlük vadesi gelen soru hedefini tamamladığınızda seri başlar ve her başarılı günde seri gün sayacınız <strong>+1 gün</strong> artar.
                            </p>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 0.6rem; align-items: flex-start;">
                        ...
                        <div>
                            ...
                            <p style="margin: 0.15rem 0 0 0; font-size: 0.83rem; color: var(--text-secondary); line-height: 1.45;">
                                Günlük vadesi gelen soruları çözdüğünüzde veya vadesi gelen soru bulunmadığı günlerde seriniz otomatik korunur. Çalışmaya ara verdiğiniz günlerde ise aktif Dondurma Jetonlarınız devreye girerek serinizin sıfırlanmasını engeller.
                            </p>
                        </div>
                    </div>

                    <div style="display: flex; gap: 0.6rem; align-items: flex-start;">
                        <span style="font-size: 1.1rem; line-height: 1;"><span style="color: #7dd3fc;">❄️</span></span>
                        <div>
                ...
                <div style="display: flex; flex-direction: column; gap: 0.65rem;">
                    <div style="display: flex; gap: 0.6rem; align-items: flex-start;">
                        ...
                        <div>
                            <strong style="font-size: 0.88rem; color: var(--text-primary);">Seri Nasıl Oluşturulur?</strong>
                            <p style="margin: 0.15rem 0 0 0; font-size: 0.83rem; color: var(--text-secondary); line-height: 1.45;">
                                Kart üzerindeki ⚙️ ikonundan kaynaklarınızı seçtikten sonra, seçili kaynaklardan günlük vadesi gelen soru hedefini tamamladıkça Odak Seri sayacınız <strong>+1 gün</strong> artar.
                            </p>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 0.6rem; align-items: flex-start;">
                        ...
                        <div>
                            <strong style="font-size: 0.88rem; color: var(--text-primary);">Seri Nasıl Korunur?</strong>
                            <p style="margin: 0.15rem 0 0 0; font-size: 0.83rem; color: var(--text-secondary); line-height: 1.45;">
                                Seçili kaynaklardaki günlük vadesi gelen soruları çözerek veya soru bulunmayan günlerde otomatik olarak seriniz korunur. Çalışamadığınız günlerde Odak Dondurma Jetonları serinizi muhafaza eder.
                            </p>
                        </div>
                    </div>
...
export function showFreezeTokenModal(type) {
    if (type === 'global') {
        const title = '<span style="color: #7dd3fc;">❄️</span> Genel Seri Dondurma Hakları';
        const html = `
            <div style="font-size: 0.88rem; color: var(--text-primary); text-align: left; display: flex; flex-direction: column; gap: 0.85rem;">
                <div style="background: rgba(125, 211, 252, 0.1); padding: 0.75rem 0.9rem; border-radius: 8px; border-left: 3px solid #7dd3fc;">
                    <strong style="color: #7dd3fc; font-size: 0.92rem;"><span style="color: #7dd3fc;">❄️</span> 1. Mavi Kar Tanesi</strong>
...
export function showContinuityTargetModal(type) {
    if (type === 'global') {
        const liveQ = buildQuestionPool() || [];
        const overdueCount = getDailyOverdueSnapshot(liveQ);
        const title = '🎯 FSRS Günlük Hedef Detayı';
        const html = `
            <div style="font-size: 0.88rem; color: var(--text-primary); text-align: left; display: flex; flex-direction: column; gap: 0.85rem;">
                <div style="background: var(--bg-hover); padding: 0.75rem 0.9rem; border-radius: 8px; border-left: 3px solid var(--primary-color);">
                    <strong style="color: var(--primary-color); font-size: 0.9rem;">FSRS Vadesi Gelen Sorular: ${overdueCount}</strong>
                    <p style="margin: 0.25rem 0 0 0; color: var(--text-secondary); font-size: 0.83rem; line-height: 1.45;">
                        Sistem hafıza eğrinize göre bugün tekrar etmeniz gereken soruları analiz eder. Vadesi gelen soru sayısı 15'ten fazla olduğunda günlük hedeflenen seri soru sayısı 15 olarak belirlenir.
                    </p>
                </div>
            </div>
        `;
