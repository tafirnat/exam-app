...
export function buildQuestionPool(options = {}) {
    ...
    AppState.sources.forEach(s => {
        ...
        s.questions.forEach(q => {
            ...
            /* A suspended question stays in the map - the stats screen still
               lists it, the user can still open and edit it - but it is not
               drawn for a test. That is the whole of what suspending does: it
               takes the question out of rotation without touching a single
               figure, so nothing is lost and it comes back the moment the user
               un-suspends it. See leech.js. */
            if (inScope && !isSuspended(AppState.stats[`${s.id}_${q.id}`])) {
                rawQuestions.push(entry);
            }
        });
    });
...
export function prepareRetake(historyEntry, onlyIncorrect = false) {
    ...
    /* A retake reaches outside the active set on purpose, and a suspended
       question is still a question the user asked to see again. Scope 'all'
       keeps the map complete either way; the pool filter is what suspension
       acts on, and this path does not draw from the pool. */
    buildQuestionPool();
