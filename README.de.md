...
- **Automatischer Ordner beim Import**: Bitten Sie die KI, ein Set in einen eigenen Ordner zu legen – sie schreibt `"folder": "ITIL 4 Foundation"` in `exam_metadata`. Beim Import landet das Set im Ordner mit diesem Namen – ein vorhandener wird wiederverwendet (Groß-/Kleinschreibung und Akzente egal), sonst wird er angelegt. Zugeordnet wird über den **Namen**, die Datei muss also keine Ordner-IDs kennen, und alle Teile einer Serie landen im selben Ordner. Optional: Schalter **Ordnerhinweise anwenden** unter *Aus der Zwischenablage einfügen* aktivieren (standardmäßig aus; dann wird das Feld ignoriert). Gilt nur beim ersten Import – verschieben Sie das Set später, bleibt es dort. Beim Teilen können Sie den Ordnernamen mitsenden.
- **Ordner per ID**: Alternativ zielt `"folderId": "folder_..."` direkt auf einen vorhandenen Ordner (ID über die Schaltfläche `id: ...` im Ordner-Verwaltungsmodal kopieren); sie hat Vorrang vor `folder`.
