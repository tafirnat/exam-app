...
- **İçe aktarırken otomatik klasör**: AI'dan seti kendi klasörüne koymasını isteyin; `exam_metadata` içine `"folder": "ITIL 4 Foundation"` yazar. İçe aktarmada set bu adı taşıyan klasöre girer — varsa o kullanılır (büyük/küçük harf ve aksan fark etmez), yoksa oluşturulur. Eşleşme **ada** göre, yani dosyanın klasör id'lerinizi bilmesi gerekmez ve bir serinin bütün parçaları aynı klasörde toplanır. İsteğe bağlı: *Panodan Al* düğmesinin altındaki **Klasör ipuçlarını uygula** anahtarını açın (varsayılan kapalı; kapalıyken alan yok sayılır). Yalnız ilk içe aktarmada uygulanır — seti sonra taşırsanız taşıdığınız yerde kalır. Bir seti paylaşırken klasör adını eklemeyi seçebilirsiniz.
- **Id ile klasör**: Alternatif olarak `"folderId": "folder_..."` mevcut bir klasörü doğrudan hedefler (id'yi klasör düzenleme modalındaki `id: ...` düğmesinden kopyalayın); `folder`'dan önceliklidir.
